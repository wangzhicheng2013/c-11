...
SockectUtility SockectUtility::sockectUtility;
 ...
 * @return socket fd
 * */
int SockectUtility::InitSocket(unsigned char type)
{
	int sock_fd = -1;
	switch (type)
	{
 ...
 * @return socket fd
 * */
int SockectUtility::BindSocket(const SocketConfig &config)
{
...
bool SockectUtility::CloseSocket(int &sock_fd)
{
	if (sock_fd >= 0)
	{
		close(sock_fd);
