 ...
 * @return true if set ok
 * */
bool RedisClient::SetNxValue(const char *key, const char *value, int timeout)
{
	if (nullptr == conn)
	{
		cerr << "redis client conn is null...!" << endl;
	...
	redisReply *reply = (redisReply *)redisCommand(conn, "set %b %b EX %d NX", key, keylen, value, valuelen, timeout);
	if (nullptr == reply)
	{
		cerr << "reply is null...!" << endl;
		return false;
 ...
 * @return true if get ok
 * */
bool RedisClient::DelValue(const char *key)
{
	if (nullptr == conn)
	{
		cerr << "redis client conn is null...!" << endl;
	...
	bool succ = true;
	if (0 != reply->integer || 1 != reply->integer)
	{
		succ = false;
		cerr << "redis delete failed...!" << endl;
	}
	freeReplyObject(reply);
	reply = nullptr;
	return succ;
